...
export default function NavBar() {
  ...
  return (
    <div className="flex bg-[#FFFDF8] justify-between items-center backdrop-blur-sm px-4 md:px-8 lg:py-6 lg:px-16 py-4">
      <button 
      ...
      <div className="font-nunito text-[#63676A] font-bold text-base hidden md:flex justify-center md:gap-6 lg:gap-10 xl:gap-12 2xl:gap-15">
        {paths.map((path) => (
          <NavLink
            ...
            to={path.path}
            className={({ isActive }) =>
              `relative after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-[#47898E] after:transition-all after:duration-300 hover:text-[#47898E] hover:after:w-full ${isActive ? 'text-[#47898E] after:w-full' : ''}`
            }
          >
            {path.name}
          </NavLink>
          ...
          <div className="flex flex-col items-center gap-8 mt-12">
            {paths.map((path) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `font-nunito text-xl font-semibold ${isActive ? 'text-[#47898E]' : 'text-[#0D393E]'}`
                }
              >
                {path.name}
              </NavLink>
...
const base = "font-nunito w-40 text-base rounded-xl py-2 border transition-all duration-300 hover:shadow-lg";
